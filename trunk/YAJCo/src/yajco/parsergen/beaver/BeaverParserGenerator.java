package yajco.parsergen.beaver;
...
import yajco.model.grammar.NonterminalSymbol;
import yajco.model.grammar.Symbol;
import yajco.model.grammar.TerminalSymbol;
import yajco.model.grammar.bnf.Alternative;
import yajco.model.grammar.bnf.Grammar;
import yajco.model.grammar.bnf.Production;
...
import yajco.semlang.translator.SemLangToJavaTranslator;
...
public class BeaverParserGenerator {
	...
	public void generateFrom(Language language, Grammar grammar, PrintStream writer) {
...
//		}
	...
	private void writeTerminals(PrintStream writer) {
		...
		List<TerminalSymbol> terminals = new ArrayList<TerminalSymbol>(grammar.getTerminals().values());
	...
	private void writeTypes(PrintStream writer) {
		for (TerminalSymbol terminal : grammar.getTerminals().values()) {
	...
	private String operatorSpecificationToString(Integer priority) {
		...
		builder.append("%" + associativityToString(opPattern.getAssociativity()) + " ");
